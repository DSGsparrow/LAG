...
class BaselineAgent(ABC):
    def __init__(self, agent_id) -> None:
        ...
        self.actor.load_state_dict(torch.load(self.model_path, weights_only=True, map_location=torch.device('cpu')))
        self.actor.eval()
...
def test_maneuver():
    env = SingleControlEnv(config_name='1/heading')
    obs = env.reset()
    env.render(filepath="bench-control.txt.acmi")
    ...
    while True:
        ...
        pitch_rad = env.agents[uid].get_property_value(c.attitude_pitch_rad)
        roll_rad = env.agents[uid].get_property_value(c.attitude_roll_rad)
        roll = np.rad2deg(roll_rad)
        pitch = np.rad2deg(pitch_rad)

        altitude = env.agents[uid].get_property_value(c.position_h_sl_m)

        print('roll: ', roll, ', pitch: ', pitch, 'altitude: ', altitude)
