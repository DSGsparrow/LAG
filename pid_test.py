...
def pid_test():
    ...
    while 1:
        ...
        # max_output_e = 25
        # min_output_e = -25
        # normalized_output_De = 2 * (output_De - min_output_e) / (max_output_e - min_output_e) - 1
        # normalized_output_Da = 2 * (output_Da - min_output_a) / (max_output_a - min_output_a) - 1
        normalized_output_De = np.deg2rad(output_De)
        normalized_output_Da = np.deg2rad(output_Da)
        ...
        # action pid
        action = [output_Da, output_De, 20, 10]  # [滚转，俯仰，偏航，速度]
