...
class EnvImitation(BaseEnv):
    ...
    def __init__(self, config_name: str, env_id):
        ...
        self.enm_episode_data = []
        self.save_dir = f"./imitation_data/env_{self.env_id}"
    ...
    def load_task(self):
        ...
        elif taskname == 'task_for_imitation':
            self.task = TaskForImitation(self.config)
        else:
    ...
    def set_random_enemy(self):
        ...
        init_enm_heading = 0  # self.np_random.uniform(0., 360.)
        init_enm_distance = self.np_random.uniform(12000., 15000.)
        init_enm_angle = self.np_random.uniform(-105., 105.)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        # === 记录敌机状态-动作对 ===
        delta_value = self.task.baseline_agent.set_delta_value(self.agents['B0100'])
        enm_actions = [
        ...
        # 添加到当前 episode 轨迹缓存中
        self.enm_episode_data.append((enm_obs.copy(), np.array(enm_actions)))
        ...
        ego_done = np.array(dones['B0100'])  # all(all(inner) for inner in dones)
    ...
    def save_enemy_episode(self):
        if not self.enm_episode_data:
            return
        states = np.array([item[0] for item in self.enm_episode_data])
        actions = np.array([item[1] for item in self.enm_episode_data])
        save_path = os.path.join(self.save_dir, f"B0100_ep{self.episode_id:04d}.npz")
        np.savez_compressed(save_path, states=states, actions=actions)
        logging.info(f"[env{self.env_id}] Saved enemy data: {save_path}")
