...
from net.net_imitation import ImitationPolicy
...
class SingleCombatTask(BaseTask):
    ...
    def load_agent(self, name):
        ...
        match_shoot = re.fullmatch(r"shoot(\d+)", name, re.IGNORECASE)
        match_dodge = re.fullmatch(r"dodge(\d+)", name, re.IGNORECASE)
        ...
        if name == 'pursue':
            ...
            return DodgeMissileAgent(int(match_dodge.group(1)))
        elif name == 'shoot_teacher':
            return ShootTeacherAgent()
        elif name == 'shoot_imi':
            return ShootImitationAgent()
        else:
...
class DodgeMissileAgent:
    def __init__(self, dodge_ver) -> None:
        super().__init__()
        self.enm_ver = dodge_ver

        if self.enm_ver == 1:
            self.model_path = 'trained_model/dodge_missile/ppo_air_combat_dodge.zip'
        else:
            self.model_path = 'trained_model/dodge_missile/ppo_air_combat_dodge' + str(self.enm_ver) + '.zip'

        self.model = PPO.load(self.model_path, device="cuda" if torch.cuda.is_available() else "cpu")
...
class ShootAgent:
    def __init__(self, shoot_ver) -> None:
        super().__init__()
        self.enm_ver = shoot_ver

        if self.enm_ver == 1:
            self.model_path_self = 'trained_model/shoot_missile/ppo_air_combat.zip'
        else:
            self.model_path_self = 'trained_model/shoot_missile/ppo_air_combat.zip'

        self.model = PPO.load(self.model_path_self, device="cuda" if torch.cuda.is_available() else "cpu")
...
class ShootImitationAgent:
    ...
    def get_action(self, sim: AircraftSimulator):
        ...
        _action = self.model.forward(obs_tensor)
        action = _action.squeeze().detach().cpu().numpy().squeeze()
