...
class TaskForImitation(HierarchicalSingleCombatTask, SingleCombatDodgeMissileTask):
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        if self.use_baseline and agent_id in env.enm_ids:
            ...
            norm_action = self.baseline_agent.get_action(sim)

            norm_obs = self.get_obs(env, agent_id)

            # 1. 距离
            distance = norm_obs[13] * 10000  # 单位：米

            # 2. TA角（角度）
            angle_rad = norm_obs[11]
            angle_deg = np.rad2deg(angle_rad)

            # 3. 敌方速度估计（用相对速度 + ego速度）
            speed = norm_obs[8] * 340  # 单位：m/s

            # 4. 高度差
            alt_diff = - norm_obs[10] * 1000  # 单位：米

            # 调用模糊决策函数
            should_attack = fuzzy_should_attack(distance, angle_deg, speed, alt_diff)

            self._shoot_action[agent_id] = should_attack
            ...
            self._shoot_action[agent_id] = 0  # action[3]
